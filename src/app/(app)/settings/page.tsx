...
export default function SettingsPage() {
  ...
  const Toggle = ({ label, icon, value, field }: { label: string; icon: string; value: boolean; field: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid var(--g-04)' }}>
      ...
      <div className={`toggle-track${value ? ' on' : ''}`} onClick={() => updateProfileField(field, !value)}>
  ...
  return (
    <div className="anim-entry">
      ...
      <div className="glass-card" style={{ borderRadius: 16, padding: '4px 16px', marginBottom: 16 }}>
        <Toggle label="Combat Alerts" icon="notifications" value={profile.notifications} field="notifications" />
        <Toggle label="Stealth Mode" icon="visibility_off" value={profile.stealth} field="stealth" />
        <Toggle label="Sound FX" icon="volume_up" value={profile.sound} field="sound" />
        <Toggle label="Confirm Deletes" icon="delete_sweep" value={profile.confirmTaskDelete} field="confirmTaskDelete" />
      </div>
      ...
      <div className="glass-card" style={{ borderRadius: 16, padding: 16, marginBottom: 16 }}>
        ...
        <select className="input-field" value={profile.currency} onChange={e => updateProfileField('currency', e.target.value)}>
          {CURRENCIES.map(c => (
      ...
      <div className="glass-card" style={{ borderRadius: 16, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <select className="input-field" value={profile.startHour} onChange={e => updateProfileField('startHour', Number(e.target.value))}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <select className="input-field" value={profile.endHour} onChange={e => updateProfileField('endHour', Number(e.target.value))}>
              {Array.from({ length: 24 }, (_, i) => (
                <option key={i} value={i}>{(i % 12 || 12) + (i >= 12 ? ' PM' : ' AM')}</option>
              ))}
            </select>
